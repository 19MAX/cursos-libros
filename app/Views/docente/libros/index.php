...
<?= $this->section('title') ?>Mis Libros<?= $this->endSection() ?>
<?= $this->section('page_title') ?>Mis Libros<?= $this->endSection() ?>
<?= $this->section('page_description') ?>Gestiona tus libros publicados y en revisión<?= $this->endSection() ?>
...
foreach ($libros as $libro) {
    ...
    if ($libro['estado'] === 'pendiente')
        $pendientes++;
    if ($libro['estado'] === 'rechazado')
        $rechazados++;
    $puntajeTotal += $libro['puntaje_asignado'] ?? 0;
}
?>
<div class="admin-dashboard-grid mb-6">
    <div class="admin-dashboard-card">
        <div class="flex items-center justify-between">
            <div>
                <div class="admin-dashboard-stat"><?= $total ?></div>
                <div class="admin-dashboard-label">Total Libros</div>
            </div>
    ...
    <div class="admin-dashboard-card">
        <div class="flex items-center justify-between">
            <div>
                <div class="admin-dashboard-stat"><?= $aprobados ?></div>
    ...
    <div class="admin-dashboard-card">
        <div class="flex items-center justify-between">
            <div>
                <div class="admin-dashboard-stat"><?= $pendientes ?></div>
    ...
    <div class="admin-dashboard-card">
        <div class="flex items-center justify-between">
            <div>
                <div class="admin-dashboard-stat"><?= number_format($puntajeTotal, 2) ?></div>
...
<div class="admin-card">
    <div class="admin-card-header">
        <div class="flex items-center justify-between">
            <div>
                <h3 class="text-lg font-semibold admin-text-primary">Lista de Libros</h3>
                <p class="admin-text-secondary text-sm">Todos tus libros registrados</p>
            </div>
            <a href="<?= base_url('docente/libros/create') ?>" class="admin-btn-primary">
            ...
            <tbody>
                <?php if (!empty($libros)): ?>
                    <?php foreach ($libros as $libro): ?>
                        <tr>
                            <td><?= esc($libro['titulo_libro']) ?></td>
                            <td><?= esc($libro['autores']) ?></td>
                            <td><?= esc($libro['editorial']) ?></td>
                            <td><?= esc($libro['fecha_publicacion']) ?></td>
                            <td><?= number_format($libro['puntaje_asignado'], 2) ?></td>
                            <td>
                                <?php
                                $estadoClass = '';
                                $estadoText = '';
                            ...
                            <td>
                                <?php if (!empty($libro['archivo_libro'])): ?>
                                    <a href="<?= base_url($libro['archivo_libro']) ?>" target="_blank"
                                        class="admin-btn-icon text-blue-600 hover:text-blue-700" title="Ver PDF">
                            ...
                            <td>
                                <?php if (!empty($libro['portada_libro'])): ?>
                                    <img src="<?= base_url($libro['portada_libro']) ?>" alt="Portada"
                                        class="h-12 w-10 object-cover rounded shadow" />
                                <?php else: ?>
                                    <span class="text-gray-400">No disponible</span>
                                <?php endif; ?>
                            </td>
                            <td>
                                <div class="flex items-center space-x-2">
                                    <?php if ($libro['estado'] === 'pendiente'): ?>
                                        <a href="<?= base_url('docente/libros/edit/' . $libro['id']) ?>" class="admin-btn-icon"
                                            title="Editar">
                                        ...
                                        <button type="button" class="admin-btn-icon text-red-600 hover:text-red-700"
                                            onclick="confirmarEliminar(<?= $libro['id'] ?>)" title="Eliminar">
                                    ...
                                    <?php else: ?>
                                        <span class="text-sm text-gray-500 dark:text-gray-400">
                                            ...
                                            <?php elseif ($libro['estado'] === 'rechazado'): ?>
                                ...
                                <form id="delete-form-<?= $libro['id'] ?>"
                                    action="<?= base_url('docente/libros/delete/' . $libro['id']) ?>" method="post"
                                    style="display: none;">
                                    <?= csrf_field() ?>
                                </form>
                            </td>
...
<script>
    if (document.getElementById("export-table") && typeof simpleDatatables.DataTable !== 'undefined') {
        const exportCustomCSV = function (dataTable, userOptions = {}) {
            ...
            if (!csv) return false
            const defaults = { download: true, lineDelimiter: "\n", columnDelimiter: ";" }
            const options = { ...defaults, ...clonedUserOptions }
            const separatorRow = Array(dataTable.data.headings.filter((_heading, index) => !dataTable.columns.settings[index]?.hidden).length).fill("+").join("+");
        ...
        const table = new simpleDatatables.DataTable("#export-table", {
            ...
            labels: {
                ...
                noRows: "No hay libros para mostrar",
